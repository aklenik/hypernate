...
[Google Java Format](https://github.com/google/google-java-format); the exact configuration lives
in the `spotless { }` block of `lib/build.gradle.kts`. CI fails if code is not formatted, so always
run:
